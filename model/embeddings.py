...
class InterSignalEmbeder(nn.Module):
    def __init__(self, in_dim=1280, d_model=128):
        super().__init__()
    ...
    def forward(self, batch):
        a = self.norm(batch['a_mint'].float())
        b = self.norm(batch['b_mint'].float())
...
class IntraSignalEmbeder(nn.Module):
    def __init__(self, in_dim=1536, d_model=128):
        super().__init__()
    ...
    def forward(self, batch):
        a = self.norm(batch['a_esm3'].float())
        b = self.norm(batch['b_esm3'].float())
